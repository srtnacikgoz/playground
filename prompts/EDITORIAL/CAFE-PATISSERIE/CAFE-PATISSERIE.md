# EDITORIAL CAFÉ & PATISSERIE

**Artisan Café ve Pastane Editorial Fotoğrafçılığı**

Minimal, sıcak, Avrupa tarzı bakery estetiği ile rafine, doğal his. Modern editorial dergi görünümü, yavaş yaşam atmosferi, otantik ve zamansız cafe kültürü.

**Platform:** DALL-E 3 (Önerilen) | Gemini (Backup) | Midjourney (Alternatif)
**Format:** 9:16 story (1080x1920px) - Instagram Stories tam ekran dikey
**Konsept:** Editorial-style artisan café & patisserie photography - warm minimalism, authentic moments

---

## DALL-E 3 PROMPT

```
[9:16 STORY FORMAT]
Editorial-style artisan café and patisserie photography in 9:16 vertical story aspect ratio (1080x1920px) for Instagram Stories. Minimal, warm, European bakery aesthetic with a refined natural feel. Full-screen mobile vertical composition.

SCENE & COMPOSITION:
Vertical portrait composition optimized for mobile Stories - subject positioned in center-lower third, leaving breathing room at top and bottom for text overlays. 45-degree angle tabletop or intimate eye-level café moments work best for tall vertical format. Natural human interaction where appropriate: hands holding pastry, slicing cake, pouring coffee. Clean but lived-in surfaces: marble, light wood, ceramic plates, linen napkins. Generous negative space at top (20%) and bottom (15%) for Story stickers, text, and swipe-up prompts.

PRODUCTS & DETAILS:
Handcrafted pastries, tarts, breads, croissants, cakes showcasing artisanal quality. Imperfect, artisanal textures clearly visible - natural folds, organic shapes, rustic edges. No excessive styling, no artificial gloss, no plastic props. Neutral ceramic and natural material props only. Authentic bakery presentation.

LIGHTING:
Soft natural daylight streaming from large window, slightly diffused creating low contrast and gentle shadows. Warm color temperature evoking late morning or early afternoon feel. No harsh highlights, no studio flash, no dramatic lighting. Even, flattering, naturally bright illumination.

COLOR PALETTE:
Warm neutrals dominate: cream, beige, soft brown, muted caramel tones. Subtle earthy accents where appropriate: sage green, terracotta, dusty rose. Overall desaturated, cohesive tone throughout the frame. Harmonious color relationships, no jarring contrasts.

TYPOGRAPHY SPACE (if text needed):
Elegant serif or modern classic typeface area. Small, refined typography placement. Off-white or muted dark brown text zones. Editorial magazine layout feeling with plenty of whitespace, never centered-heavy. Breathing room for text overlay.

MOOD & STYLE:
Calm, cozy, slow-living atmosphere. Modern European café culture. Authentic, intimate, timeless feeling. Instagram editorial aesthetic, premium lifestyle branding. Inviting yet refined, approachable luxury.

TECHNICAL SPECIFICATIONS:
9:16 vertical aspect ratio (1080x1920 pixels) - Instagram Story dimensions. Ultra high resolution, 8k quality. Shallow depth of field where compositionally appropriate, keeping main subject sharp. Film-like softness with subtle grain texture. Natural color grading avoiding HDR look. Professional food photography standards with authentic presentation. Vertical framing maximizes mobile screen real estate.

Shot on 50mm lens perspective, natural bokeh, analog film aesthetic with Kodak Portra-like tones, soft grain, understated elegance, artisanal French bakery mood, modern luxury minimalism. Tall vertical composition filling phone screen.
```

---

## NEGATIVE PROMPT

```
landscape orientation, horizontal composition, square format, 1:1 ratio, 4:5 ratio, wrong aspect ratio, bright artificial colors, neon tones, oversaturated, over-styled food, glossy artificial finish, plastic appearance, stock photo look, generic commercial photography, hard shadows, dramatic lighting, harsh spotlight, studio flash, aggressive branding, bold typography, centered-heavy text, cluttered composition, busy background, gradient vignette, HDR processing, artificial color grading, digital render look, 3D CGI, cartoon style, props overload, modern plastic materials, stainless steel, chrome finishes, cold industrial aesthetic, fast food presentation, processed food appearance, perfect geometric arrangement, artificial symmetry, staged look, catalog photography style, lifeless composition, sterile environment, pure white background, clinical lighting, product shot isolation, watermark, low resolution, blurry, noise artifacts, compression artifacts, amateur photography
```

---

## Konsept: Editorial Café & Patisserie

**Ne yapar:** Artisan café ve pastane ürünlerini editorial dergi estetiğinde, otantik ve zamansız bir atmosferle sunar.

**Anahtar Özellikler:**
- Minimal ama karakterli kompozisyon
- Doğal günışığı, yumuşak gölgeler
- Sıcak nötr renk paleti
- İnsan etkileşimi (eller, anlar)
- Temiz ama yaşanmış yüzeyler
- Negatif alan kullanımı
- Film estetiği
- Artisan imperfection

**AI Referanstan Öğrenir:**
- Ürün tipi ve sunumu
- Doku detayları
- Renk uyumu
- Kompozisyon açısı
- Props seçimi

---

## Kullanım Senaryoları

**Primary Use Cases:**
- Instagram editorial posts
- Bakery/café social media content
- Menu photography
- Brand storytelling imagery
- Website hero sections
- Print magazine editorials
- Lifestyle brand content

**Brand Contexts:**
- Artisan bakeries
- Specialty coffee shops
- Patisserie boutiques
- Farm-to-table cafés
- European-style bakeries
- Premium lifestyle brands

**Content Types:**
- Product showcase
- Behind-the-scenes moments
- Customer experience
- Seasonal campaigns
- Menu launches
- Brand identity content

---

## Stil Varyasyonları

### 1. **Pure Minimal**
- Maksimum negatif alan
- Tek ürün odak
- Monochrome nötr palet
- Çok yumuşak ışık

### 2. **Lifestyle Moments**
- İnsan etkileşimi öncelik
- Hikaye odaklı
- Daha sıcak tonlar
- Doğal anlar

### 3. **Rustic Warmth**
- Ahşap yüzeyler
- Terracotta accent
- Daha belirgin texture
- Ev yapımı his

### 4. **French Elegance**
- Beyaz porselen
- Linen peçete
- Zarif sunum
- Şık minimalizm

---

## Beklenen Sonuç

| Özellik | Değer |
|---------|-------|
| **Format** | **9:16 vertical (1080x1920px)** |
| Açı | 45° / Eye-level (dikey için ideal) |
| Kompozisyon | Dikey rule of thirds, üst/alt boşluk |
| Işık | Soft natural daylight, diffused |
| Renk | Warm neutrals, desaturated |
| DOF | Shallow (subject sharp) |
| Arka Plan | Light wood / Marble / Linen |
| Props | Ceramic, natural materials |
| Mood | Calm, cozy, slow-living |
| Stil | Editorial magazine, Instagram premium |
| Çözünürlük | 8k, film-like grain |

---

## Tipografi Entegrasyonu (Opsiyonel)

**Typography Guidelines (if adding text):**
- **Font Style:** Elegant serif (Cormorant, Playfair) veya modern classic (Futura, Avenir)
- **Size:** Small, refined (başlığı baskın yapma)
- **Color:** Off-white (#F5F3F0) veya muted brown (#4A3F35)
- **Placement:** Negatif alan bölgelerinde, asla merkez-ağır değil
- **Spacing:** Bol whitespace, editorial dergi hissi
- **Alignment:** Asimetrik, dinamik yerleşim

**Örnek Text Overlay:**
```
"Handcrafted with care"
"Artisan bakery since..."
"Fresh daily"
"Slow baked tradition"
```

---

## Platform-Specific Notlar

### DALL-E 3
- ✅ Typography space handling mükemmel
- ✅ Natural lighting interpretation iyi
- ✅ Warm color palette başarılı
- ⚠️ Overstaging eğilimi var → "authentic, lived-in" vurgusu gerekli

### Gemini Nano Banana Pro
- ✅ Artisan texture detail çok iyi
- ✅ Natural imperfection anlayışı güçlü
- ⚠️ Soft lighting için agresif tanım gerekebilir

### Midjourney
- ✅ Aesthetic quality yüksek
- ✅ Film grain ve analog look otomatik
- ⚠️ Overly artistic olabilir → "commercial product photography" vurgusu ekle

---

## İyileştirme Notları

**Gelişim Alanları:**
- [ ] Seasonal variation prompt'ları (Christmas, Valentine's, Spring)
- [ ] Specific product focus versions (croissant-only, coffee-only, cake-only)
- [ ] Color palette variations (cool tones, monochrome, high contrast)
- [ ] Time-of-day variations (morning light, golden hour, evening)

**Test Edilen Varyasyonlar:**
- V1.0: Initial prompt (current)

---

**Versiyon:** V1.0
**Oluşturma Tarihi:** 2026-01-12
**Son Güncelleme:** 2026-01-12
**Status:** Ready for Testing
**Kategori:** Editorial Photography
**Tags:** #cafe #patisserie #editorial #minimal #warmtones #artisan #lifestyle
